...
#+END_SRC
* Sub dispatch: basic override manipulation and queries
#+begin_src sh
command_dispatch() {
...
#+end_src
* Sub dispatch: defining overrides
*** Cabal-related
#+begin_src sh :var title=(org-tangle-item)
section-cabal )
...
upstream )	   HELP "upstream ATTR [REV=HEAD]"               "Override to an upstream Github commit" || return 0
                   ...
                   github "$OP_ATTR" "$user" "$rev";;
#+end_src
* Sub dispatch: building & maintaining attributes
*** Build & results
  #+begin_src sh :var title=(org-tangle-item)
section-build )
