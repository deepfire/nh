	...
	then
        ...
        do
                ...
                nix-build ${NIX_GHC_PACKAGE_SET} -A ${erdep} ${NIX_BUILD_OPTIONS} 2>&1 \
                        | set_stdin ${LOGTY} ${over}.erdeps ${attr}
                status=${PIPESTATUS[0]}
                rm -f ${PKGDB}/current.build
                if test ${status} != 0
                then
                        ...
                        fail_classify_one_property ${over}.erdeps ${attr}
                        return 1
                fi
        done
...
EOF
                           ...
                           do echo $attr
                              case $status in
                                hackaged )   echo "  upstream: $(get_upstream_release ${attr})"; echo "  nixpkgs: $(get_nixpkgs_release ${attr})";;
                                upstreamed ) echo "  upstream: $(get_upstream_release ${attr})"; echo "  nixpkgs: $(get_nixpkgs_release ${attr})"; echo "  commits: $(url upstream-commits ${attr})";;
                                unmerged )   echo "  upstream: $(get_upstream_release ${attr})"; echo "  nixpkgs: $(get_nixpkgs_release ${attr})"; echo "  PR: $(url upstream-pull-request ${attr})";;
                              esac
