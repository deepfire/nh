...
hackage_cabal_upstream_url() {
        local attr="$1"; shift
        ret=$(hackage_cabal ${attr} | sed -n '/^[Ss]ource-[Rr]epository head/,$ p' | grep -i "location:" |
...
build() {
        ...
        coherently ${reuse_cache} ${reuse_overrides} ${attr} "${over}" override_builder ${print_drv} ${attr} "${over}"
}
    #+END_SRC
*** acme
#+BEGIN_SRC sh
