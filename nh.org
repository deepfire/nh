...
failty | failure-type )   attr="$1"; over="${2:-baseline}"; get ${FAILTYTY} "${over}" ${attr};;
faillog | failure-log )   attr="$1"; over="$2";             faillog "${over}" "${attr}";;
...
trim-single )      trim_single_property       "$@";;     ##  Attempt trimming a specific override of a given attribute
