...
EOF
                local props="src doCheck doHaddock jailbreak editedCabalFile revision postPatch extraLibs"
                for p in ${props}
...
#+begin_src sh :var title=(org-tangle-item)
section-shell )
section "${title}" shell shell-for cabal-shell fixer-shell try-fix;;
...
stepper )          HELP "stepper ATTR"         "Step through ATTR's build phases" || return 0
                   # $prePhases unpackPhase patchPhase $preConfigurePhases configurePhase $preBuildPhases buildPhase checkPhase $preInstallPhases installPhase fixupPhase $preDistPhases distPhase $postPhases
                   true;;
...
ghc )
                   HELP "ghc"                    "Shell with current GHC" || return 0
