...
EOF
        done < $(path def/over patches ${attr})
        cat <<EOF
    ];
EOF
}
...
global_output_cleanup_filter() {
        sed "s|/nix/store/[0-9a-z]-|/nix/store/HASH-|g; s|${NIXPKGS}|nixpkgs:/|" |
                grep -v "cannot build derivation"
}
  #+END_SRC
*** emit-overrides
  #+BEGIN_SRC sh
...
patch | add-patch )
                   ...
                   local url="$1";    shift
                   echo "${sha256} ${url}" >> $(path def/over patches ${OP_ATTR});;
#+end_src
*** Status
  #+begin_src sh :var title=(org-tangle-item)
section-status )
...
diff | nixpkgs-diff )
                   HELP "nixpkgs-diff [(base-head|base-master|head-master] [REF]" \
                                                   "Diff of current GHC configuration" || return 0
                   local kind="${1:-base-head}" diffspec=; shift
                   local ref="${1:-HEAD}" clean=
                   case "${kind}" in
                          tree )        fetch=  diffspec=;;
                          tree-essence )fetch=  diffspec=  clean=y;;
                          base-head )   fetch=  diffspec="$(git -C "${NIXPKGS}" merge-base HEAD origin/master) HEAD";;
                          base-master ) fetch=y diffspec="$(git -C "${NIXPKGS}" merge-base HEAD origin/master) origin/master";;
                          to-master )   fetch=y diffspec="${ref} origin/master";;
                          * ) fail "unknown diff kind '${kind}'.  Expected one of: tree, base-head, base-master, to-master";;
                   esac
                   test -z "${fetch}" ||
                           git -C "${NIXPKGS}" fetch origin
                   if test -n "${clean}"
                   then git -C "${NIXPKGS}" diff ${diffspec} ${GHC_CONFIG_FULL_PATH} | grep -v '^[+-][ ]*#\|^[ @]'
