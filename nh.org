...
EOF
                   case $(status  ${attr}) in
                           hackaged | shadowed )
                   cat <<EOF
  overide release: $(get def/hackage release ${attr})
EOF
                   ;;
                           upstreamed )
                   rev=$(get def/github rev ${repo})
...
section-meta )
section "Metadata manipulation" \
         ls-meta meta set-meta;;
...
section "Override manipulation (low level)" \
         disable enable set-explanation ls get set edit \
...
ls )               HELP "ls ATTR"                 "List attribute's overrides" || return 0
                   lsattrs    def/over  "$1";;
...
check | doCheck )
                   HELP "check ATTR"              "Disable an existing dontCheck override" || return 0
                   _set def/over doCheck         "$1";        cache_gen_override_one "$1";;

dontCheck )
                   HELP "dontCheck ATTR"          "Disable tests" || return 0
                   _set def/over doCheck         "$1" false;  cache_gen_override_one "$1";;

haddock | doHaddock )
                   HELP "haddock ATTR"            "Disable an existing dontHaddock override" || return 0
                   _set def/over doHaddock       "$1";        cache_gen_override_one "$1";;

dontHaddock )
                   HELP "dontHaddock ATTR"        "Disable Haddock generation" || return 0
                   _set def/over doHaddock       "$1" false;  cache_gen_override_one "$1";;

jailbreak | doJailbreak )
                   HELP "jailbreak ATTR"          "Turn on jailbreaking" || return 0
                   _set def/over jailbreak       "$1" true;   cache_gen_override_one "$1";;

dontJailbreak ) 
                   HELP "dontJailbreak ATTR"      "Disable an existing jailbreak override" || return 0
                   _set def/over jailbreak       "$1";        cache_gen_override_one "$1";;

chdir )
                   HELP "chdir ATTR SUBDIR"       "Change directory before build;  \"\" removes the override" || return 0
                   _set def/over chdir           "$1" "$2";   cache_gen_override_one "$1";;

extraLibs )
                   HELP "extraLibs ATTR '[ .. ]'" \
                                                  "Specify extra dependencies;  \"\" removes the override" || return 0
                   _set def/over extraLibs       "$1" "$2";   cache_gen_override_one "$1";;

section-status )
section "Attribute status classification" \
         status ls-shadowed ls-hackaged ls-upstreamed ls-unmerged ls-config;;
...
section-overhigh )
section "Override manipulation (high-level)" \
         hackage unmerged upstream;;
...
upstream )	   HELP "upstream ATTR [REV=HEAD]"               "Set the source override to an upstream Github commit" || return 0
                   attr="$1"; rev="$2"; repo=$(repoName ${attr})
                   github "$repo" "$(get def/github upstream $repo)" "$rev"
                   cache_gen_override_one "$attr";;

section-github )
section "Github-based (unmerged/upstream) override manipulation" \
         github-up set-pr set-upstream set-repoName;;
...
section "Build & results" \
         build log fail failure-type failure-log expression proof;;

build )            HELP "build [COMMON-OPTS] ATTR"          "Build a single attribute" || return 0
                   build "$@";;

log | logs )
                   HELP "log ATTR [OVER=baseline]"          "Obtain trim build logs for a single override" || return 0
                   attr="$1"; over="${2:-baseline}"; get ${LOGTY}      "${over}" ${attr};;

fail | failure )
                   HELP "failure ATTR [OVER=baseline]"      "Obtain trim failure kind of an override" || return 0
...
section-nix )
section "Nix-level inferences" \
         drv src-drv dependencies referrers;;

drv )
                   HELP "drv ATTR"                          "Store derivation for a single override" || return 0
                   attr="$1"; over="${2:-baseline}"
                   do_instantiate ${attr};;

src-drv )
                   HELP "src-drv ATTR"                      "Store source derivation for a single override" || return 0
                   attr="$1"; over="${2:-baseline}"
...
section-maintain )
section "Override database maintenance" \
         instantiate baseline trim trim-single show-trims execute-trims audit;;
...
audit )            HELP "audit [--skip-baseline] [--reuse-{overrides,cache}] [ATTR]" \
                                                "Perform sanity check on the overridden attrs (or specified subset)" || return 0
                   audit "$@";;

section-generate )
section "Generation of overrides" \
         over overrides overrides-descs cache show-cache;;
...
cache )            HELP "cache [--require-descs]" \
                   ...
                   if test -z "$*"; then cache_drop override; fi
                   cache "$@";;
...
section-miscellanea )
section "Miscellanea" \
         eval shell cabal2shell loop-hunter watch;;
