...
backtrace() {
  ...
  then
    ...
    do
      echo " $i: ${BASH_SOURCE[$i+1]}:${BASH_LINENO[$i]} ${FUNCNAME[$i]} ${BASH_ARGV[$i]}" >&2
    done
  fi
}
errexit() {
...
test_shadowed() {
	...
	test_shadow_exists "${attr}" && ! has def/meta suppress-shadow "${attr}"
}
    #+END_SRC
*** Log filtering
#+BEGIN_SRC sh
...
faillog() {
	...
	set +e
        get build/failure-log ${over} ${attr}.prebuild
        local status0=$?
        get build/failure-log ${over} ${attr}
        local status1=$?
	set -e
...
fail_classify_one_property() {
        ...
        for i in "${!sig[@]}"
        do if      { get_log ${over} ${attr}.prebuild; get_log ${over} ${attr}; } | egrep --max-count=1                  "${sig[$i]}" >/dev/null 2>&1
           then
                   echo ${i}
                   if test -z "${no_state_change}"
                   then _set ${FAILTYTY} ${over} ${attr} "$i"
                   fi
                   { get_log ${over} ${attr}.prebuild; get_log ${over} ${attr}; } | egrep --max-count=1 ${grepctx["$i"]} "${sig[$i]}" | grep -v "^$" |
...
emit_override() {
        ...
        if test -z "$(lsattrs def/over ${attr})" || get def/meta disable "${attr}" >/dev/null
        then return
        fi
        if test_shadowed "${attr}"
...
coherently() {
        local overrides=overrides.nix
        local reuse_cache=
        local reuse_overrides=
        local require_desc=
        while test $# -ge 1
        do case "$1"
           in --overrides )            overrides="$2"; shift;;
	...
	# Record the overall expression used.
        ...
        then if test -z "${reuse_overrides}"
             then emit_overrides ${reuse_cache} | tee ${PKGDB}/${EXPRTY}/baseline        > ${overrides}
             else cp -f ${overrides}                  ${PKGDB}/${EXPRTY}/baseline
             fi
        else      emit_overrides ${reuse_cache} | tee ${PKGDB}/${EXPRTY}/${attr}.${over} > ${overrides}
        fi
...
override_builder() {
        ...
        ### 1. evaluate
        ###
        local store_drv="$(instantiate ${attr})"
	local status=$?
	...
	then
		...
		{ instantiate ${attr} 2>&1 || true; } |
    			set_stdin          ${LOGTY} ${over} ${attr}.instantiate
                echo "FATAL: failed to instantiate '${attr}', without overriding '${over}'" >&2
                echo "FATAL: logs:  nh get ${LOGTY} ${over} ${attr}.instantiate" >&2
        ...
        ### 2. prebuild context
        ###
        echo -n      "$(path ${LOGTY} ${over} ${attr}.prebuild)" > ${PKGDB}/current.prebuild
        nix-shell ${NIX_GHC_PACKAGE_SET} -A ${attr} ${NIX_BUILD_OPTIONS} --no-out-link 2>&1 \
                | set_stdin ${LOGTY} ${over} ${attr}.prebuild
        local status=${PIPESTATUS[0]}
        rm -f ${PKGDB}/current.prebuild
        if test ${status} = 0
        then
                _set ${FAILTY}    ${over} ${attr}.prebuild ""
                _set ${FAILLOGTY} ${over} ${attr}.prebuild ""
                _set ${FAILTYTY}  ${over} ${attr}.prebuild ""
        else
                intermediate_failure=$(get ${LOGTY} "${over}" ${attr}.prebuild | log_filter_failure)
                echo "FATAL: failed to prebuild context for '${attr}', without overriding '${over}'" >&2
                echo "FATAL: ..apparently due to:  ${intermediate_failure}"
                echo "FATAL: logs:  nh get ${LOGTY} ${over} ${attr}.prebuild" >&2
                fail_classify_one_property ${over} ${attr}.prebuild no-state-change
                fail_classify_one_property ${over} ${attr}.prebuild no-state-change | tail -n1  | set_stdin ${FAILTYTY}  ${over} ${attr}.prebuild
                fail_classify_one_property ${over} ${attr}.prebuild no-state-change | head -n-1 | set_stdin ${FAILLOGTY} ${over} ${attr}.prebuild
                _set ${FAILTY} ${over} ${attr}.prebuild $intermediate_failure
                return 1
        fi
...
trim_one() {
        ...
        do echo -n "TRIM ${attr} -${over}>  "
           if has def/meta explanation "${over}" "${attr}"
...
baseline() {
        ...
        do
                echo "--( ${attr}"
                if ! coherently --reuse-overrides ${attr} "baseline" override_builder ${attr} "baseline"
...
loop_hunter() {
        ...
        do echo "EVAL +doCheck ${x}>  with_field_value def/over doCheck $x false evaluate --reuse-cache $x"
           if with_field_value def/over doCheck "$x" false evaluate --reuse-cache "$x"
...
show-override )    emit_override "$1";;                        ## Print the override defined by PKGDB
...
disable )          _set       def/meta disable  "$1" true      ## Disable all overrides for an attribute
                   cache_gen_override_one "$1";;
enable )           _set       def/meta disable  "$1" ""        ## Re-enable previously disabled overrides
...
evaluate )         evaluate                   "$@";;     ##  Evaluate overridden attrs                    (or those specified)
baseline )         baseline                   "$@";;     ##  Build overridden attrs                       (or those specified)
trim )             trim                       "$@";;     ##  Suggest a reduction to the override set      (or its specified subset)
execute-trim )     execute_trim               "$@";;     ##  Execute the suggestion
audit )            audit                      "$@";;     ##  Perform sanity check on the overridden attrs (or those specified)
...
## Miscellanea
##
x )                "$@";;                                ##  Passthrough, to execute anything defined.
shell )
                   overrides "$@"
