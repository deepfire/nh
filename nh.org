...
upstream() {
        ...
        then import_hackage_cabal "${attr}" >&2
        fi
        get def/github upstream ${repo}
}
test_upstreamed() {
...
upstreamOrFail() {
        local attr="$1"
        upstream "$attr" ||
                fail "unable to deduce upstream for '${attr}'"
}
url() {
        local KIND="$1"; shift
        local attr="$1"; shift
        ...
        local repo="$(repoName "$attr")"
        case "${KIND}" in
                upstream-github-cabal )      echo -n "https://raw.githubusercontent.com/$(upstreamOrFail   $attr)/$repo/${gitrev}/$subpath$attr.cabal";;
                ...
                github-prs )                 echo -n "https://github.com/$(               upstreamOrFail   $attr)/$repo/pulls";;
...
hackage_cabal_upstream_url() {
        local attr="$1"; shift
        local ret=$(hackage_cabal ${attr} | sed -n '/^[Ss]ource-[Rr]epository head/,$ p' | grep -i "location:" |
...
#+BEGIN_SRC sh
coherently() {
        local reuse_cache= reuse_overrides= require_desc=
        while test $# -ge 1
        do case "$1"
...
cache_copy() {
        ...
        do
                to="$(echo -n $from | keep_until_last_dot)".${FIELD_TO}
                cp -f "${from}" "${to}"
        done
	popd >/dev/null
}
...
#+BEGIN_SRC sh
apply() {
        tmp_ngo=$(mktemp  ${TMPDIR:-/tmp}/nh-overrides.XXXXXXXXXXXXX)
        tmp_gcfp=$(mktemp ${TMPDIR:-/tmp}/nh-overrides.XXXXXXXXXXXXX)
        emit_overrides      --format overrides         ${TARGET_NIXPKGS:+--target local}                 --output-hint ${NIX_GHC_OVERRIDES}    "$@" > ${tmp_ngo}
        mv ${tmp_ngo} ${NIX_GHC_OVERRIDES}
        cp         -f ${NIX_GHC_OVERRIDES}    ${PKGDB}
        if test -n "${TARGET_NIXPKGS}"
        then emit_overrides --format configuration-ghc ${TARGET_NIXPKGS:+--target nixpkgs} --reuse-cache --output-hint ${GHC_CONFIG_FULL_PATH} "$@" > ${tmp_gcfp}
...
import_hackage_cabal() {
        ...
        local user="$(github_repo_url_user ${url})"
        local repo="$(github_repo_url_repo ${url})"
...
github() {
        ...
        local REV="${2:-$(git ls-remote https://github.com/${user}/${repo} HEAD | cut -f1)}"
...
*** build
    #+BEGIN_SRC sh
build() {
        local limit_to_self= print_drv= reuse_cache= reuse_overrides=
        while test $# -ge 1
        do case "$1"
        ...
        coherently ${reuse_cache} ${reuse_overrides} ${_attr} "${_over}" override_builder ${print_drv} ${_attr} "${_over}"
}
    #+END_SRC
*** acme
#+BEGIN_SRC sh
...
try_fix() {
        ...
        build --limit-to-self --reuse-cache "${attr}"
}
  #+END_SRC
...
#+BEGIN_SRC sh
trim() {
        local reuse_cache= skip_baseline=
        while test $# -ge 1
        do case "$1"
           in --reuse-cache )          reuse_cache="--reuse-cache";;
              --since )                since="$2"; shift;;
...
*** Nix-level inferences
  #+begin_src sh :var title=(org-tangle-item)
section-nix )
section "${title}" drv src drv-src dependencies referrers;;
...
drv-src )          HELP "drv-src STORE-DRV"                 "Path to a Nix-stored .drv file" || return 0
                   local drv="$1"
                   nix-store --query --binding src ${drv};;
...
*** Miscellanea
  #+begin_src sh :var title=(org-tangle-item)
section-miscellanea )
section "${title}" eval loop-hunter progress watch ghc-update;;
...
progress )         HELP "progress"             "Live summary of new, complete and failing builds" || return 0
