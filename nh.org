...
#+begin_src sh :var title=(org-tangle-item)
section-meta )
section "${title}" ls-meta meta set-meta edit-meta chdir local nonlocal;;
...
*** Github-based (unmerged/upstream) override manipulation
  #+begin_src sh :var title=(org-tangle-item)
section-github )
section "${title}" github-up set-upstream set-pr set-issue set-repoName;;
...
*** Miscellanea
  #+begin_src sh :var title=(org-tangle-item)
section-miscellanea )
section "${title}" eval shell cabal2shell shell-failures loop-hunter progress watch;;
