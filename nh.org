...
log_filter_progress() {
        ...
        grep --line-buffered '^error: while evaluating the attribute\|^builder for.*failed with exit code 1\|^checking for references to.* in /nix/store/.*[0-9]\.\.\.\|setupCompileFlags: -package-db=/tmp/nix-build-.*' |
            sed 's|^error: while evaluating the attribute ‘.*’ of the derivation ‘\(.*\)’ at .*:$|EVALFAIL: \1|' |
...
get_upstream_release() {
        ...
        fetch_cabal upstream ${attr} ${gitrev} | grep -i '^version:' | sed 's/[vV]ersion:[ ]*//' | remove_nonprintable
}
get_user_release() {
        local attr="$1"
        local gitrev="${2:-master}"
        fetch_cabal user ${attr} ${gitrev} | grep -i '^version:' | sed 's/[vV]ersion:[ ]*//' | remove_nonprintable
}
      #+END_SRC
...
emit_overrides() {
        ...
        echo -n "Assembling${desc}overrides.." >&2
        ###
        ### 2. Emit overrides
        ###
        case ${format} in
...
#+BEGIN_SRC sh
apply() {
        emit_overrides      --format overrides         ${TARGET_NIXPKGS:+--target local}   "$@"               > ${NIX_GHC_OVERRIDES}
        cp      -f ${NIX_GHC_OVERRIDES}    ${PKGDB}
        if test -n "${TARGET_NIXPKGS}"
        then emit_overrides --format configuration-ghc ${TARGET_NIXPKGS:+--target nixpkgs} "$@" --reuse-cache > ${GHC_CONFIG_FULL_PATH}
