...
emit_overrides() {
        ...
        do case "$1"
           in --reuse-cache )          reuse_cache="--reuse-cache";;
              ...
              --suppress-content )     suppress_content="--suppress-content";;
              --silent )               silent="--silent";;
              --debug )                enable_debug;;
              --disable-ghc-configuration ) true;; # Already handled
              "--"* )                  fail "$0: unknown option: $1";;
              * )                      break;;
           esac
           shift
...
#+BEGIN_SRC sh
overrides() {
        emit_overrides --format overrides "$@"                       > ${NIX_GHC_OVERRIDES}
        for x in "$@"
        do if test "$x" = "--disable-ghc-configuration"
           then emit_overrides --format configuration-ghc --suppress-content --reuse-cache > ${NIXPKGS}/pkgs/development/haskell-modules/${GHC_CONFIG}
           fi; done
...
#+BEGIN_SRC sh
ghc_configuration() {
        emit_overrides --format configuration-ghc                    > ${NIXPKGS}/pkgs/development/haskell-modules/${GHC_CONFIG}
        emit_overrides --format overrides --suppress-content --reuse-cache > ${NIX_GHC_OVERRIDES}
}
#+END_SRC
*** trim
#+BEGIN_SRC sh
...
*** Github-based (unmerged/upstream) override manipulation
  #+begin_src sh :var title=(org-tangle-item)
section-github )
section "${title}" github-up set-pr set-issue set-upstream set-repoName;;
...
*** Generation of overrides
  #+begin_src sh :var title=(org-tangle-item)
section-generate )
section "${title}" over overrides ghc-configuration cache show-cache;;
...
overrides )        HELP "overrides [--reuse-cache] [--disable-ghc-configuration]" \
                           "Generate ${NIX_GHC_OVERRIDES}.  --disable-ghc-configuration neuters ${GHC_CONFIG}" || return 0
                   overrides                   "$@";;

ghc-configuration )
                   HELP "ghc-configuration [--reuse-cache]" \
                           "Generate ${GHC_CONFIG}.  Neuters ${NIX_GHC_OVERRIDES}" || return 0
                   ghc_configuration           "$@";;
