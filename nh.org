...
url() {
        ...
        case "${KIND}" in
                ...
                upstream-pull-merged )       echo -n "https://api.github.com/repos/$(     get def/github user     $repo)/$repo/pulls/$(prOrFail $repo)/merge";;
...
test_hackage_release() {
        ...
        endpoint_head hackage-package ${attr} --release ${release} | grep -q 'HTTP/1.1 200 OK'
}
      #+END_SRC
*** Github
***** fetch-cabal
...
github_pr_status() {
        ...
        local http_code="$(endpoint_head upstream-pull-merged ${attr} | grep 'HTTP/1.1')"
...
audit )            HELP "audit [--autofix] [--skip-baseline] [--reuse-{overrides,cache}] [ATTR]" \
